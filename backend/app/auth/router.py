    ...
    **Password Requirements:**
    ...
    Example:
        ...
        curl -X POST http://localhost/api/auth/change-password \\
          -H "Authorization: Bearer {access_token}" \\
          -H "Content-Type: application/json" \\
          -d '{"current_password":"Old123","new_password":"New456Pass"}'
        ```
    """
    ...
    # Hash new password
    new_password_hash = PasswordHandler.hash_password(password_request.new_password)
    ...
    new_history = password_history + [current_user.password_hash]
    # Keep only last N passwords (configured in settings)
    from app.config import settings
    current_user.password_history = new_history[-settings.password_history_count:]
