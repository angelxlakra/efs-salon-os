...
class ReceiptService:
    ...
    def format_currency(amount_paise: int) -> str:
        """Format currency without rupee symbol for thermal printer compatibility.

        Args:
            amount_paise: Amount in paise (1 rupee = 100 paise)

        Returns:
            Formatted string like "Rs. 1,234.00"
        """
    ...
    def generate_receipt_pdf(bill: Bill, db: Optional[Session] = None) -> BytesIO:
        ...
        if settings and settings.salon_tagline:
            elements.append(Paragraph(settings.salon_tagline, tagline_style))
        ...
        items_data = [[
            ...
            Paragraph("<b>Price</b>", ParagraphStyle('H', parent=styles['Normal'], fontSize=8, alignment=TA_RIGHT)),
            Paragraph("<b>Amt</b>", ParagraphStyle('H', parent=styles['Normal'], fontSize=8, alignment=TA_RIGHT))
        ]]
        ...
        for item in bill.items:
            ...
            items_data.append([
                ...
                Paragraph(ReceiptService.format_currency(item.base_price), ParagraphStyle('Price', parent=styles['Normal'], fontSize=8, alignment=TA_RIGHT)),
                Paragraph(ReceiptService.format_currency(item.line_total), ParagraphStyle('Amt', parent=styles['Normal'], fontSize=8, alignment=TA_RIGHT))
            ])

        items_table = Table(items_data, colWidths=[32 * mm, 10 * mm, 16 * mm, 16 * mm])
        ...
        totals_table = Table(totals_data, colWidths=[50 * mm, 24 * mm])
        ...
        elements.append(Spacer(1, 1 * mm))
        grand_total_data = [[
            Paragraph("<b>TOTAL:</b>", ParagraphStyle('GTL', parent=styles['Normal'], fontSize=11, fontName='Helvetica-Bold', alignment=TA_RIGHT)),
            Paragraph(f"<b>{ReceiptService.format_currency(bill.rounded_total)}</b>", ParagraphStyle('GTV', parent=styles['Normal'], fontSize=11, fontName='Helvetica-Bold', alignment=TA_RIGHT))
        ]]
        grand_total_table = Table(grand_total_data, colWidths=[50 * mm, 24 * mm])
        ...
        if bill.status.value == "posted" and bill.payments:
            ...
            for payment in bill.payments:
                payment_data.append([
                    ...
                    Paragraph(ReceiptService.format_currency(payment.amount), ParagraphStyle('PA', parent=styles['Normal'], fontSize=8, alignment=TA_RIGHT))
                ])

            payment_table = Table(payment_data, colWidths=[45 * mm, 29 * mm])
