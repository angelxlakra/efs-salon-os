...
- Both Receptionist and Owner can apply any discount amount (no role-based limit)
- Both **fixed amount** and **percentage-based** discounts are supported. Percentage discounts are calculated on the frontend as a percentage of the subtotal and converted to a paise amount before being sent to the API.

### Bill Items
- Bill items can reference either a **service** (`service_id`) or a **retail product** (`sku_id`), but not both. Exactly one must be set per line item (enforced by a database check constraint).
- COGS (Cost of Goods Sold) is tracked per bill item for both services (via material usage) and retail products.

### Tips
- Bills support an optional **tip** via `tip_amount` (in paise) and `tip_staff_id` fields on the Bill model.
- Tips are recorded at the bill level and linked to the staff member who receives them.

### Multi-Staff Contributions
- A single bill item (service) can have **multiple staff contributors** tracked via the `BillItemStaffContribution` model.
- Each contribution records: `staff_id`, `role_in_service`, `sequence_order`, `contribution_split_type` (percentage, fixed, equal, time_based, or hybrid), and the calculated `contribution_amount` in paise.
- This enables accurate commission calculation for multi-staff services (e.g., a Botox treatment involving an application specialist, wash technician, and stylist).
...
```json
{
  "items": [
    {
      "service_id": "01HXXX...",  // for services (mutually exclusive with sku_id)
      "sku_id": null,  // for retail products (mutually exclusive with service_id)
      ...
      "staff_contributions": []  // optional, for multi-staff services
    }
  ],
  ...
  "discount_amount": 5000,  // optional, in paise
  "discount_reason": "Regular customer discount",  // optional
  "session_id": "01HCCC...",  // optional, links walk-ins to bill
  "tip_amount": 5000,  // optional, tip in paise
  "tip_staff_id": "01HYYY..."  // optional, staff receiving tip
}
```
...
- Each item must have either `service_id` or `sku_id` (not both)
- All service_ids and sku_ids must exist and be active
...
- [ ] Can create bill with retail products (sku_id) alongside services
- [ ] Bill-level discount applies correctly (both fixed and percentage)
...
- [ ] Tips are recorded and linked to staff
- [ ] Multi-staff contributions are tracked per bill item
