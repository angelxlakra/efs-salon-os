...
### Step 7: Set Up HTTPS (Required for Mobile Barcode Scanning)

Modern mobile browsers (Chrome, Safari) **require HTTPS** to access device cameras. Without HTTPS, the barcode scanner feature will **not work** on mobile devices (phones, tablets). This step is critical if you plan to use mobile devices for inventory barcode scanning.
...
The script will:
- Auto-detect your server's local IP address
- Generate a self-signed SSL certificate (valid for 365 days)
- Update the nginx configuration to serve over HTTPS
- Configure automatic HTTP-to-HTTPS redirect

After the script completes:

```bash
# Update .env to include HTTPS in CORS origins
# Add https:// variants to CORS_ORIGINS, e.g.:
# CORS_ORIGINS=http://localhost,https://localhost,http://192.168.1.50,https://192.168.1.50
...
sudo ufw allow 443/tcp  # Required if HTTPS is enabled
sudo ufw enable

# CentOS
sudo firewall-cmd --permanent --add-service=http
sudo firewall-cmd --permanent --add-service=https  # Required if HTTPS is enabled
sudo firewall-cmd --reload
```

### Network Access

Access the application using the server's IP address (e.g., `http://192.168.1.50`) or `http://localhost` from the server itself.
...
1. **From Windows PC:**
   - Open browser
   - Go to: `http://192.168.1.50` (your server IP)
...
- [ ] Only port 80 (and 443 if HTTPS enabled) is exposed
...
- [ ] HTTPS configured for mobile barcode scanning (run `scripts/setup-https.sh`)
...
**Access URLs:**
- Web Interface: http://localhost (or https://localhost if HTTPS enabled)
