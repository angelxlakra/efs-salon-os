...
.\scripts\setup-auto-forward.ps1
```
...
| Task | Command |
|------|---------|
| Fix external access | `.\scripts\wsl-port-forward.ps1` (as Admin) |
| Make it permanent | `.\scripts\setup-auto-forward.ps1` (as Admin) |
| Diagnose issues | `.\scripts\diagnose-network.ps1` |
...
- `scripts/wsl-port-forward.ps1` - Main port forwarding script (run after each reboot)
- `scripts/setup-auto-forward.ps1` - One-time setup for automatic port forwarding
- `scripts/diagnose-network.ps1` - Troubleshooting and diagnostics
- `WSL2-NETWORK-SETUP.md` - Detailed documentation

## Common Questions

**Q: Do I need to run this every time Windows restarts?**
A: Yes, unless you run `scripts/setup-auto-forward.ps1` to make it automatic.
...
1. Run `.\scripts\diagnose-network.ps1` to identify the issue
2. Check `WSL2-NETWORK-SETUP.md` for detailed troubleshooting
3. Review Docker logs: `wsl docker compose logs`

---

**TL;DR**: Run `.\scripts\wsl-port-forward.ps1` as Administrator. That's it.
