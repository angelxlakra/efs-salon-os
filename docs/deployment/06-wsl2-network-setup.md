...
- Make sure you ran `scripts/wsl-port-forward.ps1` as Administrator
...
1. **Automatic (recommended)**: Run `scripts/setup-auto-forward.ps1` once
2. **Manual**: Run `scripts/wsl-port-forward.ps1` after each reboot
...
**For LAN access:**
1. Run `scripts/wsl-port-forward.ps1` as Administrator
2. Access from other devices: `https://192.168.x.x` (Windows IP)
3. Optional: Run `scripts/setup-auto-forward.ps1` for automatic startup
...
- Manual: Run `scripts/wsl-port-forward.ps1` again
- Automatic: If you ran `scripts/setup-auto-forward.ps1`, it happens automatically
